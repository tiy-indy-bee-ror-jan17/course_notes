class User < ActiveRecord::Base
  ...
  # def self.want_to_be_instructors
  #   where(wants_to_be_instructor: true)
  # end

  # User.want_to_be_instructors.where(first_name: "Bob").order("create_at DESC").limit(5)

  def prepend_protocol
    if photo_url && photo_url[0..3] != "http"
      self.photo_url = "http://#{photo_url}"
    end
  end
  ...
  def all_damn_courses
    (courses_taught + courses_taken).uniq
  end
